...
<section id="catalogueControllers" class="flex flex-col items-center justify-center pb-40">
  ...
  @forelse($gamepads as $gamepad)
      <div class="flex flex-col items-center justify-center hover:underline">
        <a href="{{ route('product-details.show', $gamepad->gamepad_id) }}" class="flex items-center justify-center transition duration-500 transform border border-black rounded-lg h-96 hover:scale-110 dark:border-gray-100">
            <img src="{{ asset('assets/images/' . $gamepad->gamepad_image) }}" class="m-1 w-sm max-xl:w-xs h-fit">
        </a>
        <a href="{{ route('product-details.show', $gamepad->gamepad_id) }}" class="max-w-sm pt-6 font-semibold cursor-pointer 2xl:text-2xl max-2xl:text-md">
          {{ \Illuminate\Support\Str::limit($gamepad->gamepad_name, 15, '...') }}
        </a>
        <a href="{{ route('product-details.show', $gamepad->gamepad_id) }}" class="2xl:text-2xl max-2xl:text-md">
          ${{ number_format($gamepad->price, 2) }}
        </a>
      </div>
      @empty
      <p class="mt-10 text-lg text-center col-span-full">
        No gamepads found{{ request('query') ? ' for "' . request('query') . '"' : '' }}.
      </p>
  @endforelse
  </div>
</section>
